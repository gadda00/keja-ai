...
- **Real 404s:** the `vercel.json` catch-all rewrite (which answered *every* unknown path with HTTP 200 + the home shell) is replaced by a single regex rewrite for the non-prerendered user sections; unknown paths now fall through to `out/404.html`, which the prerender pass re-titles ("Page not found · Keja AI", `noindex` with the conflicting googlebot meta stripped, and a back-to-home link). The deploy smoke test pins the contract: unknown → 404, `/tokenize/` → 200 with its own title, `/finance` → 200.
- **Gates:** 318 tests across 23 files (16 new: canonical/breadcrumb URL forms, section-catalogue parity, vercel.json contract) · typecheck + lint clean · artifact verification green incl. the new "all 16 catalogue sections prerendered" assertion.
