...
### 4.13 Wave-14/15 additions (pipeline repair + the inventory data-split)

- **Wave-14 — the severed delivery pipeline repaired (2026-09-18):** the same-day "replace Bun with Node" conversion of the build scripts had been reverted (node cannot import the shared TypeScript catalogues, and the converted `.mjs` fallbacks swallowed the failures and returned empty arrays) — but the revert restored only the script files. `package.json`, `vercel.json` and two test suites still referenced `scripts/prerender.mjs` / `generate-trust-anchor.mjs`, which no longer existed: **every Vercel deploy since 2026-09-18 05:12 UTC failed at the build step**, and the committed `public/trust-anchor.json` had been flattened to a literal `{}` — the verifiable trust layer had no manifest. Fixed: the three TS-importing pipeline steps (sitemap / prerender / trust-anchor) run under bun again (bun is guaranteed on Vercel — it is the installCommand), the trust anchor was regenerated from live data (87 listings, engines 2026-09-13.1), and `tests/buildParity.test.ts` gained a guard that fails any future `node scripts/prerender*` regression — pinning the exact breakage class.
- **Wave-15 — the inventory data-split (the documented §8.5 milestone):** the KejaApp shell chunk statically carried the entire 87-listing catalogue because *two* shell-graph consumers imported `@/lib/inventory` (Home's featured section, the NotificationsBell's alert sweep) and *one* transitive chain dragged `properties.ts` through `investmentScore` (PropertyCard → confidence layer defaulting to the platform catalogue). The split: (1) `src/data/home-subset.json` — a **generated** 20 kB artifact (`scripts/generate-home-subset.ts`, under bun) carrying the 8 featured picks + the two precomputed stats, byte-exact pinned by `tests/homeSubset.test.ts` and regenerated by every Auto-Pilot ingest; (2) `areaInsights` extracted from `properties.ts` into `src/data/areaInsights.ts` so the shell graph never touches the catalogue module; (3) `investmentScore` split — the pure factor engine (`investmentFactors`) for the boot-critical card chip, the inventory-requiring confidence layer now explicit at every call site; (4) the notifications sweep dynamically imports the inventory *after* first paint (1.5 s debounce unchanged — verified live: the alert pipeline still fires). **First-paint critical JS: 1,134 kB → 977 kB (−157 kB)** — shell chunk 348 → 194 kB, the catalogue now a lazy-only chunk; new verify-artifacts gates: a first-paint critical-JS ratchet (1000 kB) and a named "no listing ids in the boot HTML" check. Live headless walkthrough: home paints the 8 picks + honest stats (87+ listings / 65 trust≥80 — user submissions still counted live), Discover renders 80 cards from the lazy chunk, the detail view's investment panel shows the confidence layer ("Based on 7 comparable listings in Kilimani"), the seeded saved-search sweep produced 3 notifications — zero console errors.
- **Known remaining boot weight (deliberate):** entry framework chunks (507 kB — React/Next/framer-motion) and zod (281 kB, needed at shell init by the auth + store validation seams). The structural fix for both is the Phase-2 backend migration (server-side auth drops the client zod seam; framework weight is what a SPA costs). Not papered over — measured, budgeted, documented.
- **Gates:** 546 tests / 42 files (7 new subset pins + 1 pipeline-runtime guard) · typecheck + lint clean · artifact verification green (111 URLs, 87-listing anchor, entry 507 kB ≤ 525 kB, critical 977 kB ≤ 1000 kB, inventory lazy).
...
5. **Inventory data-split — DONE (wave-15, 2026-09-18):** the KejaApp shell chunk statically carried the entire 87-listing inventory (auto-listings JSON 180 kB + authored properties 68 kB, minified into the chunk that `Home` eagerly imports), and zod (274 kB) rode along because `auth.tsx` needs it at shell init — ~1.12 MB raw / ~350 kB gzip of critical JS before first paint. The split landed as designed: `src/data/home-subset.json` (generated, 20 kB — 8 featured picks + precomputed stats), `areaInsights` extracted into its own module, the investment-score confidence layer decoupled from the catalogue, and the notifications sweep deferring its inventory import until after first paint. First-paint critical JS: **1,134 kB → 977 kB (−157 kB)**, now ratcheted by a new verify-artifacts gate (5d). Remaining known boot weight: entry framework chunks (507 kB) and zod (281 kB, the documented auth-init constraint — server-side auth migration is the structural fix).
