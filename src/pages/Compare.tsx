...
export default function Compare() {
  usePageMeta(
    'Compare Properties',
    'Side-by-side comparison of Keja-verified properties: price, yields, Investment Score, trust and mortgage estimates.'
  );
  ...
  const shareUrl = items.length
    ? `${window.location.origin}${import.meta.env.BASE_URL}compare?ids=${items.map((p) => p.id).join(',')}`
    : '';
  const shareCompare = () => {
    navigator.clipboard
      ?.writeText(shareUrl)
  ...
  const rows: { label: string; render: (p: (typeof items)[number]) => ReactNode }[] = [
      ...
      render: (p) => {
        ...
        return (
          <span className="inline-flex items-baseline gap-1.5">
            <span
              className={`rounded-md px-2 py-0.5 font-display text-sm font-bold ${scoreTone(s.overall).chip}`}
            >
              {s.overall.toFixed(1)}
            </span>
    ...
    },
    {
      ...
      render: (p) =>
        p.grossYieldEstimate ? (
          <span className="inline-flex items-center gap-1.5 font-semibold text-emerald-700">
            <TrendingUp className="h-3.5 w-3.5" />
            {p.grossYieldEstimate}% p.a.
          </span>
        ) : (
    ...
    <div className="container-luxe py-10">
      ...
      <div className="mt-3 flex flex-wrap items-start justify-between gap-3">
        <p className="max-w-2xl text-sm leading-relaxed text-ink-muted">
          Up to four properties, one honest table: price, yields, Investment Score™, trust and
          mortgage estimates — the numbers that actually decide a purchase, computed the same way
          for every listing.
        </p>
        {items.length > 1 && (
          <button
            ...
            <table className="w-full min-w-[640px] border-collapse text-sm">
              ...
              <thead>
                <tr>
                  <th
                    scope="col"
                    className="sticky left-0 z-10 bg-white p-4 text-left align-bottom text-xs font-bold uppercase tracking-wider text-ink-muted"
                  >
                    Property
                  </th>
                  {items.map((p) => (
                    <th key={p.id} scope="col" className="min-w-[220px] p-4 text-left align-bottom">
                      <div className="relative">
                        ...
                        <Link to={`/properties/${p.id}`} className="group block pr-6">
                          ...
                          <p className="mt-2 font-display text-sm font-bold leading-snug text-ink group-hover:text-gold-700">
                            {p.title}
                          </p>
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={r.label} className={i % 2 === 0 ? 'bg-cream/40' : ''}>
                    <th
                      scope="row"
                      className="sticky left-0 z-10 bg-inherit p-4 text-left text-xs font-bold uppercase tracking-wider text-ink-muted"
                    >
                      {r.label}
                    </th>
                    {items.map((p) => (
      ...
      <section className="card-luxe mt-10 p-6">
        ...
        <p className="mt-2 text-sm leading-relaxed text-ink-muted">
          Investment Score™ weighs location intelligence, yield, trust, liquidity and growth
          potential — every property is scored on the same seven factors, so a 7.8 apartment in
          Kilimani and a 7.8 villa in Karen faced identical tests. Mortgage figures assume 20% down,
          13.5% p.a. over 15 years (ESTIMATE — your bank quotes the binding rate). Gross yields are
          marketplace estimates; net yields after service charges and vacancies run 1.5–2.5 points
          lower.
        </p>
      </section>
    </div>
