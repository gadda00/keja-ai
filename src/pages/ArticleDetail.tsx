...
export default function ArticleDetail() {
  ...
  if (!article) {
    return (
      <div className="container-luxe flex min-h-[60vh] flex-col items-center justify-center py-20 text-center">
        ...
        <Link to="/insights" className="btn-gold mt-6">
          Back to Insights
        </Link>
      </div>
    );
  ...
  const related = ARTICLES.filter((a) => a.slug !== slug && a.category === article.category).slice(
    0,
    2
  );
    ...
    <article className="bg-white">
      ...
      <div className="container-luxe max-w-3xl py-12">
        {article.blocks.map((b, i) =>
          b.h ? (
            <h2 key={i} className="mt-10 font-display text-2xl font-bold text-ink first:mt-0">
              {b.h}
            </h2>
          ) : b.list ? (
            ...
            <blockquote
              key={i}
              className="my-10 border-l-4 border-gold-400 bg-cream/60 px-6 py-5 font-display text-lg italic leading-relaxed text-ink"
            >
              “{b.quote}”
            </blockquote>
          ) : (
            <p key={i} className="mt-4 text-[15px] leading-[1.85] text-ink-soft first:mt-0">
              {b.p}
            </p>
          )
        )}
        ...
        <div className="mt-12 rounded-2xl bg-ink p-6 text-center">
          ...
          <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-white/60">
            Run the numbers yourself — verified listings, Investment Scores and the full calculator
            suite.
          </p>
          <div className="mt-5 flex flex-wrap justify-center gap-3">
            ...
            <Link
              to="/invest"
              className="btn-outline !bg-transparent !text-gold-300 !border-gold-500/60"
            >
              Open calculators
            </Link>
          </div>
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              {related.map((a) => (
                <Link
                  key={a.slug}
                  to={`/insights/${a.slug}`}
                  className="card-luxe card-luxe-hover p-5"
                >
