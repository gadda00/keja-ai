...
import { FileClock, Info, Search, ShieldAlert, TriangleAlert } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col gap-5">
          ...
          <ul className="divide-y divide-gold-100">
            {filtered.map((a) => {
              ...
              return (
                <li
                  key={a.id}
                  className="flex items-start gap-4 px-5 py-4 transition hover:bg-gold-50/40"
                >
      ...
      <p className="text-[11px] leading-relaxed text-ink-faint">
        ...
        listing decisions, user role changes, partner approvals and feed operations are all recorded
        with actor, target and timestamp. In production this trail is immutable and exportable for
        due diligence (investor data room, blueprint Ch.19).
      </p>
    </div>
  );
}
