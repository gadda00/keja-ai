 ...
 */
import { ArrowDownRight, ArrowUpRight, CalendarDays, CandlestickChart, Info } from 'lucide-react';
...
export default function SecondaryMarket() {
  ...
  return (
    <div className="space-y-8">
      <section className="card-luxe p-6 sm:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-ink-muted">
              ...
              exit without selling a whole building. This simulated book shows how Keja Tokenize
              will surface price discovery, depth and distributions once licensed venues approve KPT
              trading. No real orders are matched.
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 overflow-x-auto">
          <table className="w-full min-w-[760px] text-left text-sm">
            ...
            <tbody>
              {tradable.map((p) => {
                const book = books.get(p.id) ?? orderBook(p);
      ...
      {/* sell ticket */}
      {sellId ? (
        <section className="card-luxe border-2 border-gold-300 p-6" aria-label="Sell tokens">
          {(() => {
                ...
                <div className="flex-1">
                  <p className="font-display text-lg font-bold text-ink">
                    ...
                    <span className="text-sm font-medium text-ink-muted">
                      · best bid ${book.bids[0].price.toFixed(2)}
                    </span>
                  </p>
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    ...
                    <input
                      ...
                      value={sellAmount}
                      onChange={(e) =>
                        setSellAmount(Math.min(max, Math.max(1, parseInt(e.target.value) || 1)))
                      }
                  ...
                  <button
                    onClick={() => {
                      try {
                        ...
                        toast({
                          title: `Sold ${r.tokens.toLocaleString()} ${p.tokenSymbol}`,
                          description: `Proceeds $${r.proceedsUsd.toLocaleString()} · tx ${r.txHash.slice(0, 10)}… (simulated)`,
                        });
                        setSellId(null);
                      } catch {
