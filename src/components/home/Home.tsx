...
function FeaturedProperties() {
  // Wave-15 data-split: the featured set is frozen at build time
  // (scripts/generate-home-subset.ts — highest trust, views tiebreak, no
  // sold stock) instead of importing the full 87-listing catalogue into the
  // boot-critical shell. The rule is pinned byte-exact by
  // tests/homeSubset.test.ts and regenerated by every Auto-Pilot ingest.
  const featured = HOME_FEATURED;
  const [userListings] = useUserListings();
  const total = HOME_STATS.totalListings + userListings.length;
    ...
    <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <Button variant="outline" className="font-bold" onClick={() => navigate('/properties')}>
          View all {total}+ listings <ArrowRight className="ml-1 h-4 w-4" aria-hidden />
        </Button>
      </div>
...
function SocialProof() {
  // Wave-15 data-split: the two inventory-derived metrics are precomputed at
  // build time; user-submitted listings (device-local) still count live so
  // the strip never understates what a visitor has published.
  const [userListings] = useUserListings();
  const metrics = useMemo(
    () => [
      { value: `${HOME_STATS.totalListings + userListings.length}+`, label: 'Listings on the marketplace' },
      { value: `${HOME_STATS.verifiedHigh}`, label: 'Verified properties (Trust ≥ 80)' },
