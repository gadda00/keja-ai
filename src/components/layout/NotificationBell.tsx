            ...
            <div className="flex items-center gap-1">
              {notifs.length > 0 && (
                <>
                  <button
                    onClick={markAllRead}
                    className="inline-flex items-center gap-1 rounded-lg px-2 py-1 text-[11px] font-semibold text-ink-muted hover:bg-gold-50 hover:text-gold-700"
                  >
          ...
          <div className="max-h-80 overflow-y-auto" role="region" aria-label="Notification list">
            {notifs.length === 0 ? (
              <p className="px-4 py-8 text-center text-sm text-ink-muted">
                No notifications yet. Save a search on the marketplace and we&apos;ll alert you when
                new matches arrive.
              </p>
            ) : (
              notifs.slice(0, 12).map((n) => (
                ...
                >
                  <span
                    className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${n.read ? 'bg-gold-200' : 'bg-gold-600'}`}
                    aria-hidden="true"
                  />
                  <Link
                  ...
                  >
                    <span
                      className={`block text-[13px] ${n.read ? 'font-medium text-ink-soft' : 'font-bold text-ink'}`}
                    >
                      {n.title}
                    </span>
                    ...
                    <span className="mt-1 block text-[10px] font-semibold uppercase tracking-wide text-ink-muted/70">
                      {fmt(n.createdAt)}
                    </span>
                  </Link>
