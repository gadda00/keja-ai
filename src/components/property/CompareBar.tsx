    ...
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-gold-200 bg-white/95 shadow-[0_-8px_30px_rgba(25,22,18,0.12)] backdrop-blur">
      <div className="container-luxe flex items-center gap-3 py-3">
        ...
        <ul className="hidden min-w-0 flex-1 items-center gap-2 sm:flex">
          {items.map((p) => (
            <li key={p!.id} className="flex max-w-[220px] items-center gap-1.5 truncate rounded-full bg-gold-50 px-3 py-1 text-xs font-medium text-ink-soft ring-1 ring-gold-100">
        ...
        <div className="ml-auto flex items-center gap-2">
          <button onClick={() => setCompare([])} className="hidden text-xs font-semibold text-ink-muted hover:text-red-600 sm:block">
            Clear
          </button>
