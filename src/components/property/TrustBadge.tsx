...
export default function TrustBadge({ score, size = 'md' }: { score: number; size?: 'sm' | 'md' }) {
  ...
  if (tier.tone === 'watch') {
    return (
      <span
        className={`inline-flex items-center rounded-full bg-amber-100 px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-amber-700 ${sizes.badge}`}
      >
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full bg-red-100 px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-red-700 ${sizes.badge}`}
    >
