...
export function autoListingToProperty(l: AutoListing): Property {
  const feedName = l.auto.source.startsWith('feed:')
    ? l.auto.source.split(':')[2]
    : 'market scanner';
  return {
    ...l,
    ...
    images: l.images.map((img) =>
      img.startsWith('http') || img.startsWith('data:') ? img : asset(img)
    ),
...
export function autoPilotStats() {
  ...
  const avgQuality = AUTO_LISTINGS.length
    ? Math.round(
        AUTO_LISTINGS.reduce((acc, l) => acc + l.auto.qualityScore, 0) / AUTO_LISTINGS.length
      )
    : 0;
