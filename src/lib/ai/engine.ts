...
function parseBudget(t: string): { max?: number; min?: number } {
  ...
  const plain = t.match(
    /(?:under|below|max|budget of|upto|up to|less than|within)\s*(?:kes?|kshs?\.?|shillings?)?\s*(\d+(?:\.\d+)?)(m|million|k)?/
  );
  ...
  const rangeM = t.match(
    /(\d+(?:\.\d+)?)\s*(?:m|million)?\s*(?:-|–|to)\s*(\d+(?:\.\d+)?)\s*(m|million)/
  );
  if (rangeM) {
...
function matchProperties(q: ParsedQuery): Property[] {
  let list = [...marketInventory()];
  if (q.areas.length) {
    const countyHit = q.areas.some((a) => a === 'Nairobi');
    list = list.filter(
      (p) =>
        q.areas.includes(p.area) ||
        q.areas.includes(p.county) ||
        (countyHit && p.county === 'Nairobi')
    );
  }
  if (q.type) list = list.filter((p) => p.type === q.type);
  if (q.bedrooms) list = list.filter((p) => (p.bedrooms ?? 0) >= (q.bedrooms ?? 0));
  if (q.purpose === 'rent') list = list.filter((p) => p.purpose.includes('rent'));
  if (q.maxBudget) {
    list = list.filter((p) =>
      q.purpose === 'rent'
        ? (p.rentEstimate ?? p.price) <= (q.maxBudget ?? Infinity)
        : p.price <= (q.maxBudget ?? Infinity)
    );
  }
  if (q.minBudget && q.purpose !== 'rent') list = list.filter((p) => p.price >= (q.minBudget ?? 0));
  // Trust-weighted ranking: score first, recency as tiebreaker
  return list
    .sort((a, b) => b.trustScore - a.trustScore || b.listedAt.localeCompare(a.listedAt))
    .slice(0, 4);
}
...
export class KejaAI {
  ...
  respond(input: string): AIResponse {
    ...
    // Language switching
    if (
      /\b(kiswahili|swahili|kiswahili\s*please|sawa|sw)\b/.test(t) &&
      /\b(speak|switch|change|badilisha|talk|kiswahili)\b/.test(t)
    ) {
      this.language = 'sw';
      return {
        text: L.sw.greeting,
        quickReplies: ['Natafuta nyumba Kilimani', 'Uwekezaji Kitengela', 'English please'],
      };
    }
    ...
    if (/\benglish\b/.test(t)) {
      this.language = 'en';
      return {
        text: 'Switched to English. How can I help you find home today?',
        quickReplies: ['Find property', 'Investment analysis', 'How verification works'],
      };
    }

    // Greetings
    if (
      /^(hi|hello|hey|habari|habari yako|jambo|hola|salut|good (morning|afternoon|evening)|niaje|sasa|mambo)\b/.test(
        t
      ) ||
      t === 'start' ||
      t === 'get started'
    ...
    if (
      /(^|\b)(qualif(y|ication|ied)? me|find me a home|match me|what should i buy|start (the |my )?(process|journey)|guide me)(\b|$)/.test(
        t
      )
    ) {
    ...
    if (
      /(show me (my )?matches|my matches|based on my profile|my budget)/.test(t) &&
      this.lastQualification
    ) {
      const q = parseQuery(
        `${this.lastQualification.interest ?? ''} ${this.lastQualification.budget ?? ''} ${this.lastQualification.timeline ?? ''}`
      );
      const matches = matchProperties({
        ...q,
        purpose: /rent|kodi/i.test(this.lastQualification.budget ?? '') ? 'rent' : q.purpose,
      });
      return this.searchAnswer(q, matches.length ? matches : marketInventory().slice(0, 4));
    }

    // Fresh inventory — newest listings first
    if (
      /(new (listings?|arrivals?|posting?s?|on the market)|fresh (listings?|stock)|just (listed|added)|latest (listings?|properties)|what'?s new)/.test(
        t
      )
    ) {
    ...
    if (
      /(verify|verification|verified|trust|legit|scam|fraud|fake|ardhisasa|title|kuthibitisha|ghal|how do you (check|know)|safe)/.test(
        t
      )
    ) {
    ...
    if (
      /(tokeni[sz]|fractional|fraction of|tokeni[sz]ation|buy tokens|digital tokens|blockchain|ledger|kmy|kujua kuhusu tokens?)/.test(
        t
      )
    ) {
    ...
    if (/(kjai|kpt|utility token|property token|token economics|tokenomics|two.?token)/.test(t)) {
      return {
        ...
        meta: [
          {
            label: 'FACT',
            text: 'Two-token model (KJAI utility + KPT property) is the documented blueprint architecture',
          },
          {
            label: 'ASSUMPTION',
            text: '1B KJAI supply is a planning figure pending legal & economic validation',
          },
        ],
        quickReplies: [
          'Open Keja Tokenize',
          'How does the tokenization lifecycle work?',
          'Is this regulated in Kenya?',
        ],
      };
    }
    ...
    if (/(investment score|score|rating|how do you rank|trust score|scoring)/.test(t)) {
      return {
        ...
        meta: [
          {
            label: 'FACT',
            text: 'Score weights and factors are published on every property detail page',
          },
          {
            label: 'ASSUMPTION',
            text: 'Location demand bands and type norms are illustrative baselines',
          },
        ],
        quickReplies: [
          'Show me top-scored properties',
          'How does verification work?',
    ...
    if (
      /(partner|agency partner|become a partner|list globally|global listings|international|diaspora|syndicat|feed)/.test(
        t
      )
    ) {
      return {
        ...
        meta: [
          { label: 'FACT', text: 'Five-channel supply strategy per the marketplace architecture' },
        ],
    ...
    if (
      /(invest|roi|yield|return|appreciation|worth it|good deal|rental income|cash ?flow|mapato|uwekezaji)/.test(
        t
      )
    ) {
    ...
    // Mortgage / financing
    if (
      /(mortgage|loan|financing|bank|deposit|interest rate|hire purchase|kcb|stanbic|installment)/.test(
        t
      )
    ) {
    ...
    if (
      /(process|how do i buy|steps|legal|lawyer|due diligence|stamp duty|transfer|advocate|closing|buying process)/.test(
        t
      )
    ) {
    ...
    if (byId) {
      ...
      const p = marketInventory().find((x) => x.id.toUpperCase() === wanted);
    ...
    return {
      text: L[this.language].fallback,
      quickReplies: ['2BR Kilimani under 15M', 'Land in Kitengela', 'Investment analysis'],
    };
  }
  ...
  private propertyAnswer(p: Property): AIResponse {
    const trust =
      p.trustScore >= 75
        ? `✅ **Trust Score ${p.trustScore}/100** — Verified by Keja`
        : `⚠️ **Trust Score ${p.trustScore}/100** — ${p.trustSignals.some((s) => s.status === 'fail') ? 'FLAGGED by our trust layer. I do not recommend proceeding without extra due diligence.' : 'Under review.'}`;
    const yieldLine =
  ...
  private newListingsAnswer(): AIResponse {
    ...
    return {
      ...
      meta: [
        {
          label: 'FACT',
          text: 'Sorted by listing date — verification status checked at publication.',
        },
      ],
      quickReplies: [
        'Tell me more about the newest one',
        'How do listings get verified?',
        'Show investment deals',
      ],
    };
  ...
  private tokenizeAnswer(): AIResponse {
    return {
      ...
      meta: [
        {
          label: 'FACT',
          text: 'Live demo offerings: 5 Nairobi assets, 7.0–8.0% net yields, $10 tokens.',
        },
        {
          label: 'ASSUMPTION',
          text: 'Yields are projections — not guaranteed — and tokens are illiquid early on.',
        },
      ],
  ...
  private investmentAnswer(input: string): AIResponse {
    ...
    if (area && areaInsights[area]) {
      ...
      const areaProps = marketInventory()
        .filter((p) => p.area === area && !isRentalPrice(p.price))
        .slice(0, 2);
      ...
      return {
        ...
        meta: [
          {
            label: 'FACT',
            text: 'Market price bands reflect verified agency listings on Keja this quarter.',
          },
          {
            label: 'ESTIMATE',
            text: 'Yields depend on achieved rent and occupancy — your actuals may differ.',
          },
        ],
        quickReplies: [
          'Compare with another area',
          'Run numbers on a specific listing',
          'Open the investment calculator',
        ],
      };
    }
    // General investment education
    const topYield = [...marketInventory()]
      .filter((p) => p.grossYieldEstimate && p.trustScore >= 85)
      .sort((a, b) => (b.grossYieldEstimate ?? 0) - (a.grossYieldEstimate ?? 0))
      .slice(0, 3);
    return {
      ...
      quickReplies: [
        'Full projection on the top pick',
        'What about land instead?',
    ...
    if (/(ardhisasa|title|land search|deed)/.test(t)) {
      return {
        ...
        meta: [
          {
            label: 'FACT',
            text: 'Title-check status shown on each listing comes from documented searches, not guesses.',
          },
        ],
        quickReplies: [
          'Show me fully verified listings',
          'What fraud signals do you detect?',
          'What about off-plan safety?',
        ],
      };
    }
    return {
      ...
      quickReplies: [
        'Show a flagged example',
        'What is Ardhisasa?',
        'Show me only verified listings',
      ],
    };
  }
  ...
  private continueQualification(input: string): AIResponse {
    ...
    if (!state) return this.respond(input);
    const t = input.trim();
    switch (state.step) {
      case 1:
        state.data.name = t;
      ...
      case 4: {
        ...
        const budgetHint =
          budgetNum >= 5
            ? ` With a budget around ${state.data.budget}, you have real options.`
            : '';
        return {
          ...
          meta: [
            {
              label: 'FACT',
              text: `Profile captured: ${state.data.name} · ${temperature} lead · AI qualification flow`,
            },
          ],
