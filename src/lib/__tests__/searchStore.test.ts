...
describe('matchesSearch parity with marketplace filters', () => {
  it('matches by free-text query across title/area/county/id/type/agency', () => {
    ...
    expect(
      hits.every((h) =>
        `${h.title} ${h.area} ${h.county} ${h.id} ${h.type} ${h.agency}`.toLowerCase().includes('kilimani'),
      ),
    ).toBe(true)
  })
  ...
  it('purpose=rent only matches rent-listed properties', () => {
    ...
    expect(PROPERTIES.filter((x) => matchesSearch(x, f)).every((h) => h.purpose.includes('rent'))).toBe(true)
  })
})
