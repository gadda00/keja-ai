...
const baseListing = {
  ...
  images: [
    '/images/props/apartment_0.jpg',
    '/images/props/apartment_1.jpg',
    '/images/props/interior_0.jpg',
  ],
...
describe('quality gate (round-2 criticals)', () => {
  it('hard-fails price=0 listings — never auto-publishes them', () => {
    ...
    const priceCheck = q.checks.find(
      (c: { label: string; status: string }) => c.label === 'Price-band screen'
    );
...
describe('dedupe + signature graveyard (round-2 critical A1)', () => {
  ...
  it('allows genuinely different listings through', () => {
    const different = {
      ...baseListing,
      id: 'KJA-A0098',
      title: 'Test 4-Bedroom Villa in Karen',
      type: 'villa',
      area: 'Karen',
      price: 45000000,
      bedrooms: 4,
